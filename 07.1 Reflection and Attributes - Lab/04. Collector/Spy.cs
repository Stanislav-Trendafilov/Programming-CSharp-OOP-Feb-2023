...
namespace Stealer
{
    public class Spy
    {
        ...
        public string CollectGettersAndSetters(string className)
        {
            ...
            MethodInfo[] methodsInfo = type.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static|BindingFlags.NonPublic);

            foreach (var method in methodsInfo.Where(x => x.Name.StartsWith("get")))
            {
                sb.AppendLine($"{method.Name} will return {method.ReturnType}");
            }

            foreach (var method in methodsInfo.Where(x => x.Name.StartsWith("set")))
            {
